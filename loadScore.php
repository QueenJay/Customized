            ...
            <div id="SIDE" style="margin-top:2%;margin-left:1%;margin-right:3%;">
                ...
                <div class="last" style="background:white; margin-top:20px; width:100%;">
                    <div style="padding:20px">
                        <form action="server.php" method="post" id="loadscore" id="usersForm">
                            <label class="label" name="class">Class</label> 
                            <select class="inn" name="selectClass" id="selectClass" onchange="postNum()"; required>
                                <option value="">---Please Select an Option---</opion>
                            ...
                            <label class="label" name="" id ="">Admission Number</label> 
                            <select class="inn" name="admissionNum" id="admissionNum" required>

                                <option value=""> -----Select Students Please------</opion>
                                <?php if(isset($_GET['edit'])){
                                    $class = $_GET['edit'];
                                    $sql="SELECT * FROM students WHERE classOfEntry = '$class'";
                                    $query=$connect->display($sql);
                                    while ($row = $query->fetch(PDO::FETCH_ASSOC)) {  ?>
                                        <option value="<?php echo $row['admissionNum'];?>"><?php echo $row['admissionNum'];?></opion>
                                <?php }} ?>

                            </select> <br/>
                            <label class="label">Subject</label> <input class="inn" type="text" name="subject" required> <br/>
                            <label class="label">Exam Score</label> <input class="inn" type="number" name="exam" min="1" max="60" placeholder="Not more than 60" required> <br/>
                            <label class="label">Test Score</label><input class="inn" type="number" name="test" min="1" max="30" placeholder="Not more than 30" required> <br/>
                            <label class="label">Home-work</label> <input class="inn" type="number" name="homeWork" min="1" max="10" placeholder="Not more than 10" required> <br/>
                            <button class="inn" name="savescore" class="btn" type="submit" onclick="submit()" style="margin:20px; margin-left:30px;">Save</button>
                        </form> 
                    </div>
...
<script src="jquery.js"></script>
 <script>
    ...
    function submit() { 
        ...
        function postNum(){
            var el=document.getElementById('selectClass');
            var elxx=el.value ;
            el.value= elxx;
            // localStorage.setItem('selectedtem', elxx);
            window.location.href = "loadScore.php?edit="+elxx;
        }
